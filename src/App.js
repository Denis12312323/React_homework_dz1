...
const Person = ({ person }) => {
  const gender = person.gender !== 'n/a' ? person.gender : 'Unkown'
  const hairColor = person.hair_color !== 'n/a' && person.hair_color !== 'none' ? person.hair_color : 'Unkown'
    ...
    <div>
      ...
      {characters.map((character, index) => (
        <div key={index}>
          ...
          <p>Image:  <img src={character.image} alt={character.name} style={{ maxWidth: '100px' }} /></p>
          <h4>Episodes:</h4>
          <ul>
            {character.episode.map((episode, idx) => (
              <li key={idx}>{episode.name} - {episode.air_date}</li>
            ))}
          </ul>
        </div>
      ))}
  ...
  return (
    <div className="App">
      <header className="App-header">
        <div>
          <h1>Population</h1>
          <PopulationData data={data.data}></PopulationData>
        </div>
        <div>
          <h2>Star Wars</h2>
          <PeopeList people={swapiData.results}></PeopeList>
        </div>
        <div>
